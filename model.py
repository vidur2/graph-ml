...
class Graph_RNN_structure(nn.Module):
    ...
    def init_hidden(self,len=None):
        if len is None:
            return Variable(torch.ones(self.batch_size, self.hidden_size, 1))
        else:
            hidden_list = []
            for i in range(len):
                hidden_list.append(Variable(torch.ones(self.batch_size, self.hidden_size, 1)))
            return hidden_list
    ...
    def forward(self, x, teacher_forcing, temperature = 0.5, bptt=True,bptt_len=20, flexible=True,max_prev_node=100):
        ...
        #     y_pred = Variable(torch.zeros(x.size(0), x.size(1), x.size(2)))
        #     y_pred_long = Variable(torch.zeros(x.size(0), x.size(1), x.size(2)))
        #     x_step = x[:, 0:1, :]
        ...
        thresh = 0.5
        x_thresh = Variable(torch.ones(x_pred_sample.size(0), x_pred_sample.size(1), x_pred_sample.size(2)) * thresh)
...
# batch_size = 8
# output_size = 4
# generator = Graph_RNN_structure(hidden_size=16, batch_size=batch_size, output_size=output_size, num_layers=1)
# for i in range(4):
#     generator.hidden_all.append(generator.init_hidden())
#
# x = Variable(torch.rand(batch_size,1,output_size))
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        for m in self.modules():
            if isinstance(m, GraphConv):
                ...
                # m.weight.data = torch.rand([m.input_dim, m.output_dim])*init_range
                # print('find!')
...
# x = Variable(torch.rand(1,8,10))
# adj = Variable(torch.rand(1,8,8))
# model = GCN_encoder_graph(10,10,10)
# y = model(x,adj)
# print(y.size())
...
def preprocess(A):
    ...
    D = Variable(torch.zeros(A.size(0),A.size(1),A.size(2)))
...
class GCN_generator(nn.Module):
    ...
    def forward(self,x,teacher_force=False,adj_real=None):
        ...
        batch_num = x.size(0)
        node_num = x.size(1)
        adj = Variable(torch.eye(node_num).view(1,node_num,node_num).repeat(batch_num,1,1))
        adj_output = Variable(torch.eye(node_num).view(1,node_num,node_num).repeat(batch_num,1,1))
        ...
        for i in range(1, node_num):
            ...
            # x = x / torch.norm(x, p=2, dim=2, keepdim=True)
        # one = Variable(torch.ones(adj_output.size(0), adj_output.size(1), adj_output.size(2)) * 1.00).float()
        # two = Variable(torch.ones(adj_output.size(0), adj_output.size(1), adj_output.size(2)) * 2.01).float()
...
#             x = Variable(torch.rand(batch,i,4))
#             adj = Variable(torch.eye(i).view(1,i,i).repeat(batch,1,1))
#             # print('x', x)
#             # print('adj', adj)
#
#             # y = generator(x)
...
#### test code ####
# x = Variable(torch.randn(1, 256, 1))
# decoder = CNN_decoder(256, 16)
# y = decoder(x)
...
class Graphsage_Encoder(nn.Module):
    ...
    def forward(self, nodes_list, nodes_count_list):
        ...
        nodes_list[0] = Variable(nodes_list[0])
        ...
        nodes_features_farther = Variable(torch.Tensor(nodes_features.size(0), nodes_count.size(1), nodes_features.size(2)))
        i = 0
        ...
        # 2-hop feature
        # nodes original features to representations
        nodes_list[1] = Variable(nodes_list[1])
        ...
        # 1-hop feature
        # nodes original features to representations
        nodes_list[2] = Variable(nodes_list[2])
        ...
        # own feature
        nodes_list[3] = Variable(nodes_list[3])
